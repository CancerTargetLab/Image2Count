...
def create_attn_mask(sample, max_channels):
    # True = padding
    ...
    mask = np.full((max_channels), True)
    mask[0 : sample.shape[0]] = False
    
    return mask

def pad_images(sample, max_channels):
    paddings = -1.0 # retrieved as a constant from repo (?)
    return np.pad(
        sample,
        ((0, max_channels - sample.shape[0]), (0, 0), (0, 0), (0, 0)),
        mode="constant",
...
def extract_idx(path):
    ...
    elif '_mask.npy' in stem:
        stem = stem.split('_mask.npy')[0]
